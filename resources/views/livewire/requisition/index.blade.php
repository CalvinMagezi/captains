<div>
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            Per page:
            <select wire:model="perPage" class="form-select w-full sm:w-1/6">
                @foreach($paginationOptions as $value)
                    <option value="{{ $value }}">{{ $value }}</option>
                @endforeach
            </select>

            @can('requisition_delete')
                <button class="btn btn-indigo ml-3 disabled:opacity-50 disabled:cursor-not-allowed" type="button" wire:click="confirm('deleteSelected')" wire:loading.attr="disabled" {{ $this->selectedCount ? '' : 'disabled' }}>
                    {{ __('Delete Selected') }}
                </button>
            @endcan

            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="Requisition" format="csv" />
                <livewire:excel-export model="Requisition" format="xlsx" />
                <livewire:excel-export model="Requisition" format="pdf" />
            @endif




        </div>
        <div class="w-full sm:w-1/2 sm:text-right">
            Search:
            <input type="text" wire:model.debounce.300ms="search" class="w-full sm:w-1/3 inline-block" />
        </div>
    </div>
    <div wire:loading.delay>
        Loading...
    </div>

    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        <th class="w-9">
                        </th>
                        <th class="w-28">
                            {{ trans('cruds.requisition.fields.id') }}
                            @include('components.table.sort', ['field' => 'id'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.name') }}
                            @include('components.table.sort', ['field' => 'name'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.for') }}
                            @include('components.table.sort', ['field' => 'for.unique_key'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.amount') }}
                            @include('components.table.sort', ['field' => 'amount'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.specifics') }}
                            @include('components.table.sort', ['field' => 'specifics'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.latest_date') }}
                            @include('components.table.sort', ['field' => 'latest_date'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.completed_on') }}
                            @include('components.table.sort', ['field' => 'completed_on'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.approved_by') }}
                            @include('components.table.sort', ['field' => 'approved_by'])
                        </th>
                        <th>
                            {{ trans('cruds.requisition.fields.approved_on') }}
                            @include('components.table.sort', ['field' => 'approved_on'])
                        </th>
                        <th>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($requisitions as $requisition)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $requisition->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $requisition->id }}
                            </td>
                            <td>
                                {{ $requisition->name }}
                            </td>
                            <td>
                                @if($requisition->for)
                                    <span class="badge badge-relationship">{{ $requisition->for->unique_key ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                {{ $requisition->amount }}
                            </td>
                            <td>
                                {{ $requisition->specifics }}
                            </td>
                            <td>
                                {{ $requisition->latest_date }}
                            </td>
                            <td>
                                {{ $requisition->completed_on }}
                            </td>
                            <td>
                                {{ $requisition->approved_by }}
                            </td>
                            <td>
                                {{ $requisition->approved_on }}
                            </td>
                            <td>
                                <div class="flex justify-end">
                                    @can('requisition_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.requisitions.show', $requisition) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('requisition_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.requisitions.edit', $requisition) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('requisition_delete')
                                        <button class="btn btn-sm btn-indigo mr-2" type="button" wire:click="confirm('delete', {{ $requisition->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="10">No entries found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>

    <div class="card-body">
        <div class="pt-3">
            @if($this->selectedCount)
                <p class="text-sm leading-5">
                    <span class="font-medium">
                        {{ $this->selectedCount }}
                    </span>
                    {{ __('Entries selected') }}
                </p>
            @endif
            {{ $requisitions->links() }}
        </div>
    </div>
</div>

@push('scripts')
    <script>
        Livewire.on('confirm', e => {
    if (!confirm("{{ trans('global.areYouSure') }}")) {
        return
    }
@this[e.callback](...e.argv)
})
    </script>
@endpush
